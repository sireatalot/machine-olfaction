...
def find_most_similar_molecule_excluding_self(target_cid, predictions_df, threshold=0.97):
...
for index, row in tqdm(training_data.iterrows(), total=training_data.shape[0], desc="Augmenting data"):
    ...
    # Randomly choose a single molecule to substitute
    rand_cid_A = random.choice(existing_cids_A)
    rand_cid_B = random.choice(existing_cids_B)

    # Find most similar molecules
    most_similar_cid_A = find_most_similar_molecule_excluding_self(rand_cid_A, predictions)
    most_similar_cid_B = find_most_similar_molecule_excluding_self(rand_cid_B, predictions)
    ...
    new_cids_A = [most_similar_cid_A if cid == rand_cid_A else cid for cid in cids_A]
    new_cids_B = [most_similar_cid_B if cid == rand_cid_B else cid for cid in cids_B]
    ...
    label_column.extend([label_count] * 4)
...
extended_mixture_definitions_path = './data/New_Random_Definitions_0.97.csv'
extended_training_data_path = './data/New_Random_Training_Data_0.97.csv'
