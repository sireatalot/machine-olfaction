...
mixture_csv_path = "demo/Mixutre.csv"
...
# 2. Create a new primary key by concatenating Dataset and Mixture Label
mixture_df["mixture_id"] = (
    mixture_df["Dataset"].astype(str) + "_" +
    mixture_df["Mixture Label"].astype(str)
)

# 3. Move the new column to the front
cols = ["mixture_id"] + [c for c in mixture_df.columns if c != "mixture_id"]
mixture_df = mixture_df[cols]

# 4. Save the updated CSV in the same directory
output_path = "demo/Mixutre_with_primary_key.csv"
mixture_df.to_csv(output_path, index=False)
...
# ----------------------------------------------
# Determine to use which adaptive tau depending 
